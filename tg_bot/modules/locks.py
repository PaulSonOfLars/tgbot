    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(
                    "Bloccati i messaggi che contengono {} per tutti i non admin!".format(
                        args[0]
                    )
                )
                ...
                message.reply_text(
                    "Bloccati i {} per tutti i non admin!".format(args[0])
                )
                return (
                    "<b>{}:</b>"
                    "\n#LOCK"
                ...
                message.reply_text(
                    "Cosa stai cercando ti bloccare...? Usa /locktypes per la lista degli elementi bloccabili."
                )
    else:
        message.reply_text(
            "Non sono un amministratore oppure non ho i permessi di cancellazione."
        )
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(
                    "Sbloccati i messaggi che contengono {} per tutti!".format(args[0])
                )
                return (
                    "<b>{}:</b>"
                    "\n#UNLOCK"
                    "\n<b>Admin:</b> {}"
                    "\nSbloccato <code>{}</code>.".format(
                        html.escape(chat.title),
                        mention_html(user.id, user.first_name),
                        args[0],
                    )
                ...
                message.reply_text(
                    "Cosa stai cercando ti sbloccare...? Usa /locktypes per la lista degli elementi bloccabili."
                )

        else:
            bot.sendMessage(chat.id, "Cosa stai cercando ti sbloccare...?")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text(
                                "Vedo un bot, mi è stato detto di non permettergli di unirsi... "
                        ...
                        message.reply_text(
                            "Solo gli admin sono autorizzati ad aggiungere bot a questo gruppo! Fuori di qui."
                        )
            else:
                try:
                    message.delete()
...
__help__ = """
 - /locktypes: lista degli elementi bloccabili

*Solo admin:*
 - /lock <type>: blocca un elemento di un certo tipo (non diponibile in privato)
 - /unlock <type>: sblocca un elemento di un certo tipo (non diponibile in privato)
 - /locks: lista degli attuali elementi bloccati

I blocchi possono essere usati per limitare gli utenti di un gruppo.
eg:
Bloccare gli url cancellerà automaticamente tutti i messaggi contententi url, \
bloccare gli stickers farà lo stesso con tutti i messaggi di quel tipo, ecc.
Bloccare i bot impedirà ai non admin di aggiungere bot al gruppo.
"""
