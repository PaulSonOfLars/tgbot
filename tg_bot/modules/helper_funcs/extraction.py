...
def id_and_is_channel_from_reply(message):
...
def extract_user_and_text_and_is_channel(message: Message, args: List[str]) -> (Optional[int], Optional[str], Optional[bool]):
    ...
    if len(split_text) < 2:
        return id_and_is_channel_from_reply(message)  # only option possible
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        user_id, text, is_channel = id_and_is_channel_from_reply(message)
